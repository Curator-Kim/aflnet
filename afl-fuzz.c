...
EXP_ST u8  skip_deterministic,        /* Skip deterministic stages?       */
           ...
           remote_mode;               /* Missing bianary file?            */
    ...
    if (state_count > 1) {
      ...
      for(i=1; i < state_count; i++) {
        ...
        Agnode_t *from, *to;
		    ...
		    if (!from) {// the node doesn't exist
...
int send_over_network()
{
  ...
  /*sometimes not splitting up the requests may be better*/
  /*such as some implementation of modbustcp*/
  if(NOT_SPLIT_UP_REQUEST){
    u8* msgbuf = NULL;
    u32 msglen = 0;
    for(it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)){
      msgbuf = ck_realloc(msgbuf, msglen + kl_val(it)->msize);
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  MEM_BARRIER();
  if(remote_mode){
    /*not need to fork in remote mode*/
  } 
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:e:P:KEq:s:RFc:l:r")) > 0)
    ...
    switch (opt) {
      ...
      case 'r': /* remote mode, we do black box fuzz */
        if (remote_mode) FATAL("Multiple -r options not supported");
        remote_mode = 1;
        no_forkserver = 1;
        dumb_mode = 1;
        break;
  ...
  if ((optind == argc && !remote_mode) || !in_dir || !out_dir) usage(argv[0]);
  ...
  if (!remote_mode && waitpid(forksrv_pid, NULL, 0) <= 0) {
    WARNF("error waitpid\n");
  }
