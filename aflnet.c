...
#define reverse_size_end_short(s) ((s & 0xff) << 8) + ((s & 0xff00) >> 8)
/*
Split the modbustcp packet. Need to consider abnormal packets.
Just check the length field of modbus packet and we can simply parse it
the format of modbus can be found at https://github.com/M3m3M4n/modbus-fuzz-note

Most of modbustcp request packets have a length of 12bytes. 
Maybe we can simple parse it by length? 
*/
region_t *extract_requests_MODBUSTCP(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref) {
#define MODBUS_CONTROL_SIZE 8
  unsigned int region_count = 0;
  unsigned short modbus_remain_size = 0;
  region_t *regions = NULL ;
// I think the usage of mem is strange and unnecessary.
  ...
  while (cur_start < buf_size){
    ...
    if (buf_size - cur_start > MODBUS_CONTROL_SIZE){
      modbus_remain_size = (buf[cur_start + 4] << 8) + buf[cur_start + 5];
      if (buf_size - cur_start >= 6 + modbus_remain_size){
        cur_end = cur_start + 6 + modbus_remain_size - 1;
      }
      else{// possibly malformed
        cur_end = buf_size - 1;
      }
      regions[region_count - 1].start_byte = cur_start;
      regions[region_count - 1].end_byte = cur_end;
    ...
    else{// malformed, put all in one region
      regions[region_count - 1].start_byte = cur_start;
      regions[region_count - 1].end_byte = buf_size - 1;
      regions[region_count - 1].state_sequence = NULL;
      regions[region_count - 1].state_count = 0;
...
unsigned int* extract_response_codes_rtsp(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  ...
  unsigned int state_count = 0;
  char terminator[2] = {0x0D, 0x0A};
  char rtsp[5] = {0x52, 0x54, 0x53, 0x50, 0x2f}; //rtsp start
...
// we use the function code as reponse codes
// In most cases, the response packets are normal. Confidently parse it by length.
...
//     state_count += 1; 
//     state_sequence = (unsigned int *)ck_realloc(state_sequence, state_count * sizeof(unsigned int));
//     state_sequence[state_count - 1] = 0;// we have to do this... to enable state_aware_mode...
//     if (buf_size - cur_start >= MODBUS_CONTROL_SIZE){
//       state_count++;
//       state_sequence = (unsigned int *)ck_realloc(state_sequence, state_count * sizeof(unsigned int));
//       unsigned int message_code = buf[cur_start + 7];
//       modbus_remain_size = (buf[cur_start + 4] << 8) + buf[cur_start + 5];
//       if (buf_size - cur_start >= 6 + modbus_remain_size){
//         cur_end = cur_start + 6 + modbus_remain_size - 1;
//       }
...
//         cur_end = buf_size - 1;
//       }
//       memcpy((char *)&message_code + 1, buf + cur_start + 8, cur_end - cur_start  > 10 ? 3: cur_end - cur_start - 7);
//       cur_start = cur_end + 1;
//       state_sequence[state_count - 1] = message_code;
//     }
//     else{// malformed
//       // I don't know what to do... just put 0.
//       break;
//     }
//   }
//   *state_count_ref = state_count;
